...
export default function HomePageGenerator() {
  ...
  const getDinner = async (dinnerCalories: any) => {
    if (dinnerCalories != null) {
      ...
      while (!gotCalories) {
        try {
          let dinnerResponse = await fetch(
            `/api/generateDinner?calories=${dinnerCalories.text?.dinner}&ingredients=${ingredients}&userLocation=${region}&allergies=${allergies}`
          );
      ...
      while (!gotCalories) {
        try {
          let dinnerResponse = await fetch(
            `/api/generateDinner?ingredients=${ingredients}&userLocation=${region}&allergies=${allergies}`
          );
          const dinnerData = await dinnerResponse.json();
          if (dinnerData.meal && typeof dinnerData.meal.title === 'string') {
            setDinner(dinnerData.meal);
  ...
  const getSnack = async (snackNumber: any, snackCalories: any) => {
    if (snackNumber === 1) {
      if (snackCalories != null) {
        ...
        while (!gotCalories) {
          try {
            let snack1Response = await fetch(
              `/api/generateSnack?calories=${snackCalories.text?.snack1}&ingredients=${ingredients}&userLocation=${region}&allergies=${allergies}`
            );
        ...
        while (!gotCalories) {
          try {
            let snack1Response = await fetch(
              `/api/generateSnack?ingredients=${ingredients}&userLocation=${region}&allergies=${allergies}`
            );
            const snack1Data = await snack1Response.json();
            if (snack1Data.meal && typeof snack1Data.meal.title === 'string') {
              setSnack1(snack1Data.meal);
    ...
    if (snackNumber === 2) {
      if (snackCalories != null) {
        ...
        while (!gotCalories) {
          try {
            let snack2Response = await fetch(
              `/api/generateSnack?calories=${snackCalories.text?.snack2}&ingredients=${ingredients}&userLocation=${region}&allergies=${allergies}`
            );
    ...
    if (snackNumber === 3) {
      if (snackCalories != null) {
        ...
        while (!gotCalories) {
          try {
            let snack3Response = await fetch(
              `/api/generateSnack?calories=${snackCalories.text?.snack3}&ingredients=${ingredients}&userLocation=${region}&allergies=${allergies}`
            );
