...
const handler: NextApiHandler = async (req, res) => {
    ...
    const userQuery = calories ? `Step 1: I have ${ingredients}  in my pantry but im very willing to buy more ingredients, out of those, select only the ones most commonly used to make dinner. Step 2: Please select a common popular dinner recipe with ${calories} calories based on already existing cookbook articles that may or may not contain some of the pantry ingredients, Staying within a 5 calorie range of the total. Deliver in JSON format: {title: string; ingredients: string; calories: number}`
        : `Step 1: I have ${ingredients}  in my pantry but im very willing to buy more ingredients, out of those, select only the ones most commonly used to make dinner. Step 2: Please select a common popular dinner recipe based on already existing cookbook articles that may or may not contain some of the pantry ingredients. Deliver in JSON format: {title: string; ingredients: string; calories: number}`
    try {
